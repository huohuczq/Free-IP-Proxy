...
def get_recent_logs():
    try:
        ...
        if res.stdout and res.stdout.strip(): return res.stdout
    except: pass
    try:
        log_path = Path("/var/log/proxy-lite.log")
        if log_path.exists():
...
sysctl --system >/dev/null 2>&1 || sysctl -p /etc/sysctl.d/99-proxy-lite.conf >/dev/null 2>&1

# ---- OS 探测：Alpine 走 apk+OpenRC，Debian/Ubuntu 走 apt+systemd ----
if command -v apk >/dev/null 2>&1; then
    echo "[*] 检测到 Alpine Linux，使用 apk + OpenRC 适配..."
    apk add --no-cache openvpn python3 curl iproute2 iptables psmisc procps cronie
    INIT_SYS=openrc
elif command -v apt-get >/dev/null 2>&1; then
    echo "[*] 检测到 Debian/Ubuntu，使用 apt + systemd 适配..."
    apt-get update -q
    apt-get install -y openvpn python3 curl iproute2 iptables cron psmisc
    INIT_SYS=systemd
else
    echo "[!] 不支持的系统：未发现 apk 或 apt-get，退出。"
    exit 1
fi

mkdir -p /opt/proxy_lite/configs
cd /opt/proxy_lite || exit 1
...
EOF
    ...
    rc-update add proxy-lite default
